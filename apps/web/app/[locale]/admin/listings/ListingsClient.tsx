...
import { createListingAndRevalidate, updateListingStatus, deleteListingById } from './actions';

export default function ListingsClient({ initialListings, districtsList, agentsList }: { initialListings: any[], districtsList: string[], agentsList: any[] }) {
    ...
    const handleSubmit = async () => {
        ...
        const res = await createListingAndRevalidate(formData);
        ...
        if (res.error) {
            ...
            setShowForm(false);
            setFormData({ title: '', district: districtsList[0] || 'BKK1', price: '', bedrooms: '1', bathrooms: '1', size: '', floor: '', description: '', type: 'apartment', agentId: agentsList[0]?.id || '' });
        }
    };
    ...
    const handleActionModalConfirm = async () => {
        ...
        if (type === 'approve') await updateListingStatus(id, 'active');
        if (type === 'reject') await updateListingStatus(id, 'draft');
        if (type === 'delete') await deleteListingById(id);

        setActionModal(null);
    };

    const filtered = filter === 'All' ? initialListings : initialListings.filter(l => l.status.toLowerCase() === filter.toLowerCase());
    ...
    return (
        <div className="p-6 md:p-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                {[{ label: 'Total', val: initialListings.length, color: 'var(--color-brand-600)' }, { label: 'Active', val: initialListings.filter(l => l.status === 'active').length, color: 'var(--color-fresh-600)' }, { label: 'Pending Review', val: initialListings.filter(l => l.status === 'pending').length, color: 'var(--color-warn-600)' }, { label: 'Expired', val: initialListings.filter(l => l.status === 'expired').length, color: 'var(--color-danger-600)' }].map(s => (
